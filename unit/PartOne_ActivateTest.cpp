...
TEST_CASE("emptyContainers: plugin containers open empty, deposits stay, "
          "excluded bases keep loot",
          "[PartOne][espm]")
{
  auto& partOne = GetPartOne();
  auto& ref = partOne.worldState.GetFormAt<MpObjectReference>(0x20570);
  ...
  actor.AddItem(0x12eb7, 1);
  ref.PutItem(actor, { 0x12eb7, 1 });
  REQUIRE(ref.GetInventory().GetTotalItemCount() == 1);

  partOne.worldState.containerLootBaseIds = { ref.GetBaseId() };
  ref.RelootContainer();
  REQUIRE(ref.GetInventory().GetItemCount(0x12eb7) >= 1);
  REQUIRE(ref.GetInventory().GetTotalItemCount() > 1);

  partOne.worldState.emptyContainers = false;
  partOne.worldState.containerLootBaseIds.clear();
  DoDisconnect(partOne, 0);
  partOne.DestroyActor(0xff000000);
}
