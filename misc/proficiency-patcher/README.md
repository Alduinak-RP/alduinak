...
4. with `--stage`, runs `verify_r13.py`, which reads every plugin of `settings.stage.json` with `misc/esplib.py`
   ...
   `disableReferences` reference only gains Initially Disabled; a race of `races` only loses the spells the
   section removes, and every one ends without them; and `proficiency-ids.json` carries the plugin's full slot. Exit code 3 on any problem, and `verify-r13.txt` lists it.
