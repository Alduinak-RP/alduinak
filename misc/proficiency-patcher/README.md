...
- `--hotfix` runs only the steps of the hotfix list at the top of `Program.cs`: cooking, smithing, tempering,
  tailoring, factions, uncraftable, writing, racial, the enchantment magnitudes, the races, the head parts, the
  disabled references and actors, the crafting categories and the marker effects. The live plugin already holds
  what the others build. Their sweeps of the load order touch only recipes the plugin does not override yet and
