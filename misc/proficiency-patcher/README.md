...
- `--hotfix` runs only the steps of the hotfix list at the top of `Program.cs`: cooking, smithing, tempering,
  ...
  and actors, the crafting categories and the marker effects. The live plugin already holds what the others build.
  Their sweeps of the load order touch only recipes the plugin does not override yet and none a Creation Club
  plugin defines, so the tiers it ships stay as they are; the factions, uncraftable and racial rules still read
  every recipe, the plugin's own overrides included, and the tailoring `tiers` lists apply to the recipes they
  name. A named cooking or `addItems` recipe the sweep skips is not an error. A new step goes into both lists, in
  the full run's order.
...
4. with `--stage`, runs `verify_r13.py`, which reads every plugin of `settings.stage.json` with `misc/esplib.py`
   ...
   `disableReferences` reference only gains Initially Disabled; an item newly overridden for the crafting
   categories is its winner with only category keywords added; a race of `races` only loses the spells the
   section removes and changes only the passives and description it gives, and every one ends without them; and `proficiency-ids.json` carries the plugin's full slot. Exit code 3 on any problem, and `verify-r13.txt` lists it.
