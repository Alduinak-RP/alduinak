...
- `--hotfix` runs only the steps of the hotfix list at the top of `Program.cs`: cooking, smithing, tempering,
  ...
  parts, the disabled references and actors, the crafting categories and the marker effects. The live plugin already
...
3. verifies the output against the pre-cleaned input with `misc/esplib.py`: only KYWD, SPEL, MGEF, FURN,
   ...
   overrides `leveledItems` names by editor id and the REFR overrides `placements` names by form key;
