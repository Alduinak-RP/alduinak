...
- No `--next-form-id`: every record is found by editor id and reused, and a new one takes the plugin's own next
  ...
  such a record pins its id with `formId` past that block (the Eastmarch guard helmet recipe at `0x041200`); the
  r15 record diff is what caught the shift.
...
3. verifies the output against the pre-cleaned input with `misc/esplib.py`: only KYWD, SPEL, MGEF, FURN,
   ...
   records `overrides` names (items, recipes and foods by form key, own references by editor id);
...
4. with `--stage`, runs `verify_r13.py`, which reads every plugin of `settings.stage.json` with `misc/esplib.py`
   ...
   `overrides.foods` its winner with only the named effect swapped, and a reference of
