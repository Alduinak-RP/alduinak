...
The Missives mod places its notice board in every hold capital. Activating a
board opens a menu of the notices pinned there; anyone can read them, pinning
one costs **100 gold** (taken server-side, refused if the player cannot pay),
and a notice stays on the board for **7 days** before it fades on its own.

- Client piece: `skymp5-client/src/services/services/bountyBoardService.ts` (no hotkey, opened by activating a board)
...
Activation of a board is consumed by the server (`mp.onActivate`, chained the
same way `HousingSystem` chains it) and answered with the menu, so the vanilla
Missives behaviour never runs.

## Posting

- A notice costs `bountyBoardCostGold` (default 100) in gold, `baseId 0xf`.
...
- Posting requires standing within `bountyBoardMaxDistance` (default 512)
  units of the board copy the player activated, and an open board session, so
...
| Key | Default | Meaning |
|---|---|---|
| `bountyBoardCostGold` | 100 | price of pinning a notice |
