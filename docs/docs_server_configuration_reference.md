...
The list is split into collapsible sections, all collapsed at first and remembered until the game restarts: Server locations (these configured entries), Settlements (cities, towns, settlements, orc strongholds), Forts (forts, castles, civil war camps), Temples and Other. Each header shows its count and empty sections are hidden. The search covers every section and opens the ones with a match.
