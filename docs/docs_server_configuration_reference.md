...
| `gatheringProduceYield` | `{ "BeeHive": { "BeeHoneyComb": 2, "CritterBeeIngredient": 2, "BeeHiveHusk": 2 }, "BeeHiveVacant": { … } }` | `{ "<container>": { "<item editor id or hex id>": count } }` handed over instead of the container record's own contents. A container whose items do not resolve keeps its record contents |
