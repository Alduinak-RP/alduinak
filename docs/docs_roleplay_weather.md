...
region shows it. Indoors the client holds a clear sky (SkyrimClear), so an
inn's smoke hole or a ruin's open roof never shows rain, fog or a storm; the
region's weather is back the moment the player steps outside, a relog inside
an inn included. Staff can hold a weather on a region from the Personal Menu's
Admin > Weather sub-tab.
...
- Client piece: `skymp5-client/src/services/services/weatherService.ts` (applies the packet, holds a clear sky indoors, re-applies after doors and fast travel)
