...
- **What other players see:** their copy of a follower moves by the owner's movement stream and aims 0.2 s ahead along the slope (`movementApply.ts`). A new slope is taken only from steps of 16 units or more, and the height added ahead never exceeds the height change of the last step, so a short follower stopping and starting beside its owner stays on the ground on their screen.
...
- **Calling it back:** `companionCommand` / `follow` drops the target, the reverse of an attack order, and logs `CompanionSystem: {id} of {owner} ordered to follow` when there was one. The owner's client sends it with a `companionId` from command mode (E on the commanded companion or dog, prompt `{name} Follow`) and from the X menu's Follow, clears the target locally at once and stops the copy's combat, so it follows again without waiting for `companionState`.
- **Defending the owner:** when anyone damages the owner, or any pet or companion of theirs, every companion and ally of that owner targets the attacker. Only a hit the other `onHitDamageAttempt` hooks allow and that deals damage counts: a hit refused by god or ghost mode or a carrier, or one dealing no damage, starts no fight. One that is already fighting switches target at most once every 3 s. Nothing of the owner's is ever a valid target, and nothing of theirs can damage anything else of theirs.
- **Following across cells:** when the owner's client can no longer have a copy of the companion, because the owner is in another cell or outside the 3x3 grid of 4096-unit cells the server streams around the companion (`actorNeighbors`), the server moves it 64 units in front of the owner and 96 units above their feet, so it drops onto the ground instead of into a slope or the door behind them. The move is logged as `CompanionSystem: {id} brought to {owner}` and the owner is assigned as host once. Farther than 4096 units but still streamed, the owner's own engine keeps it following.
...
| `defend(ownerId, aggressorId)` | Every companion and ally of the owner targets the aggressor. Already runs on every damaging hit the other hooks allow on an owner or on one of their pets. |
