...
>   invalid name is refused with "That name will not do." and no key is cut; the label is capped at the
>   prompt's 24 characters (the server allows 32, `[A-Za-z0-9 '_-]`, brackets
>   stripped so a label can never forge a credential). Revoking ownership or
