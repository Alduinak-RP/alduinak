...
>   (KEYM 0xDB0E2 + `name` extra) that can be handed over in trade. Cut a key
>   asks for the key's name first and sends `createkey` with `name`; the item
>   is named `<label> (<TAG>[-<serial>]/<n>)`, e.g. `Breezehome Key (1A6E4-2/3)`,
>   where TAG is the primary door's form id in hex, the serial moves on at every
>   re-key and `n` is the property's cut counter (`cut` on its record), so no
>   two keys ever stack. The bracketed suffix is the credential: the server
>   parses `TAG[-serial]` back out of it, and the older `Property Key (TAG)`
>   names keep opening their doors. A `createkey` without `name` (a client
>   older than the prompt) cuts a key labelled `Property Key`. An empty or
>   invalid name is refused with "That name will not do." plus the allowed
>   characters, and no key is cut. The prompt (the pet name prompt given
>   `allowedChars`, `allowedHint` and `maxLength`) takes the server's rule: 32
>   characters of `[A-Za-z0-9 '_-]`, brackets excluded so a label can never
>   forge a credential; it shows the rule, holds OK back until the text fits,
>   and prefills the property name with other characters removed. Revoking ownership or
>   keys strips every online copy and bumps the serial, so a copy that was
>   offline or in a container stops matching. A stack of keys cut before the
>   cut number existed (`Property Key (TAG)` xN) is split when its holder's
>   actor is assigned at login, once the `keySplitOnLogin` server setting is
>   on (default off; turn it on only after the client with the new
>   SkyrimPlatformImpl.dll has shipped and named keys stay separate in game,
>   since an older client merges the split keys back): the property's `cut` moves on by N and is
>   stored first, then one inventory write swaps the stack for N keys named
>   `Property Key (TAG/n)` with the same credential. A failed write leaves the
>   stack whole (only cut numbers are lost); stale keys, stacks in containers
>   and stacks of more than 64 are left as they are.
> - Named copies in the engine (2026-09): `ExtraTextDisplayData::IsNotEqual` is
>   `return false` in the game, so the engine stacked every key (and every
>   named writing) added to an inventory onto the first one under its name,
>   and put/take requests then asked the server for N of one name ("Source
>   inventory doesn't have enough 0xdb0e2"). SkyrimPlatform's `Hooks.cpp`
>   replaces that vtable slot: two plain custom names (no message or quest
>   alias) compare by text, so differently named copies stay separate stacks
>   as they are on the server. Equally named copies still stack.
...
- **Show Title** is one checkbox per Main tab column and at most one at a time;
  ...
  `nameFor` puts it in front of chat names. The game server also puts it in
...
| --- | --- | --- |
| `propertyRequest` `{ action, target, recipient?, name? }` | Client → Server | Claim/abandon/lock/unlock/transfer/rename/createkey/revokekeys/revoke/grantcontainer on the targeted door/container; `name` is the new property name for `rename` and the key's label for `createkey` |
