...
const serverSettings = [
  ...
  { key: 'startingItems',           label: 'Starting items',        type: 'json',   group: 'Gameplay', help: 'Kit granted to fresh characters: [{ baseId, count }]. baseId as a number or "0x..." string. Default: the miner clothes; the gold comes with the first profession kit (masteryKitGold). Gold (0x0000000f) listed here is granted once per slot. Wearable kit items are equipped when character creation finishes.' },
  ...
  { key: 'masteryKitGold',          label: 'Profession kit gold',   type: 'number', group: 'Gameplay', help: 'Gold every profession\'s starting kit carries on top of its items, alchemists included. Default 50; 0 turns it off. Read at boot.' },
  { key: 'masteryKitGoldSince',     label: 'Kit gold backfill since', type: 'text', group: 'Gameplay', help: 'ISO date (or epoch ms) from which characters whose profession kit came without gold receive it once, at boot and after login. Leave empty for no backfill. Read at boot.' },
