...
const serverSettings = [
  ...
  { key: 'startingItems',           label: 'Starting items',        type: 'json',   group: 'Gameplay', help: 'Kit granted to fresh characters: [{ baseId, count }]. baseId as a number or "0x..." string. Default: the miner clothes; the gold comes with the first profession kit (masteryKitGold). Gold (0x0000000f) listed here is granted once per slot, and that character\'s profession kit then comes without gold. Wearable kit items are equipped when character creation finishes.' },
  ...
  { key: 'masteryKitGold',          label: 'Profession kit gold',   type: 'number', group: 'Gameplay', help: 'Gold every profession\'s starting kit carries on top of its items, alchemists included; a character that got gold from Starting items gets none. Default 50; 0 turns it off. Read at boot.' },
  { key: 'masteryKitGoldSince',     label: 'Kit gold backfill since', type: 'text', group: 'Gameplay', help: 'ISO date (or epoch ms) from which characters whose profession kit came without gold receive it once, at boot and after login; characters marked as having got spawn gold are skipped, but characters from before that marker existed cannot be told apart. Leave empty for no backfill. Read at boot.' },
  ...
  { key: 'bodyIdleSeconds',            label: 'PK body idle lifetime (s)',     type: 'number', group: 'Interactions', help: 'How long a PK body lies after its last take or put before it is removed with whatever is left in it. A body nobody has touched is not affected. 0 turns it off. Default 7200. Read at startup.' },
  { key: 'bodyMaxSeconds',             label: 'PK body lifetime (s)',          type: 'number', group: 'Interactions', help: 'How long the lootable body a PK leaves lies before it is removed with whatever is left in it. 0 (default) keeps it until it is emptied. Read at startup.' },
  { key: 'keySplitOnLogin',            label: 'Split old key stacks at login', type: 'bool',   group: 'Interactions', help: 'Splits a stack of property keys cut before keys were numbered (Property Key (TAG) xN) into N numbered keys when its holder logs in. Keep off until the client with the new SkyrimPlatformImpl.dll has shipped and named keys stay separate in game; an older client merges the split keys back and put/take fails. Read at startup. Default off.' },
  ...
  { key: 'bountyBoardStashBase',       label: 'Bounty board strongbox base',   type: 'text',   group: 'Interactions', placeholder: 'c674b:Skyrim.esm', help: 'CONT base (hex:Plugin desc or load-order id) placed at each board to hold the posting fees; must carry no base items. Default the vanilla ash pile.' },
  ...
  { key: 'leverLinks', label: 'Lever links', type: 'json', group: 'Advanced', help: 'Levers wired to their gate through markers the server never loads: [{ levers:["hex:File.esp", ...], target:"hex:File.esp", all?, openAnim?, closeAnim? }]. all: every lever pulled once before the target moves. A DOOR target toggles isOpen, anything else plays open/close. Default: Soljund\'s Sinkhole portcullis. [] disables. Read at boot.' },
