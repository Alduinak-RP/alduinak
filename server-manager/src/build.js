...
class Builder {
  ...
  pruneServerDeploy() {
    ...
    const keep = new Set(['world', 'server-settings.json', 'server-settings.json.prev', 'gamemode.js', 'gamemode_extensions', 'plugins', 'dist_back', 'scam_native.node', 'data', 'sign-gamemode.js', 'signing-private.pem', 'install-services.bat', 'launch_server.bat', 'readme.md', 'starter-grants.json', 'zone-spawns.json', 'housing.json', 'npc-spawns.json'])
    ...
    for (const name of entries) {
      // Timestamped backups (deploy/mongodb scripts, purges) stay next to the settings file
      if (keep.has(name.toLowerCase()) || /-\d{13}\.json$/i.test(name)) continue
      try {
